...
#define FONT_WIDTH_MARKER_COLOUR                                               \
  0x081828 /* 16 bit colours have a common granularity of 8 */
#define FONT_CHAR_COLOUR 0xf8e8d8

#define FONT_CHAR_COUNT                                                        \
  223 /* There's currently an extra slot at pixmap end                         \
         */
...
/* Variables */
int char_dimensions[FONT_CHAR_COUNT][2]; /* Character in font, width marker x1
                                            and width marker x2 */
...
/* This function :-
   ...
   * Records the variable widths of each character if width markers exist in row
 * 0 */
...
SDL_Surface *TTF_OpenFont(const char *file, int ptsize) {
  ...
  if (SDL_SetColorKey(unconverted, SDL_TRUE, // SDL_SRCCOLORKEY,
                      SDL_MapRGB(unconverted->format, 0xff, 0x00, 0xff)) < 0) {
    fprintf(stdout, "Cannot set surface colour key: %s\n", SDL_GetError());
    return NULL;
  }
  /* Convert the loaded surface to the same pixel format as the screen for fast
   * blitting */
  ...
  for (count = 0; count < FONT_CHAR_COUNT; count++) {
    ...
    char_dimensions[count][1] =
        (count + 1) * converted->w / FONT_CHAR_COUNT - 1;
    x12 = 0;
    /* If width markers exist then use those */
    for (cx = char_dimensions[count][0]; cx <= char_dimensions[count][1];
         cx++) {
      if (get_pixel(converted, cx, 0) ==
          SDL_MapRGB(converted->format, FONT_WIDTH_MARKER_COLOUR >> 16 & 0xff,
                     FONT_WIDTH_MARKER_COLOUR >> 8 & 0xff,
                     FONT_WIDTH_MARKER_COLOUR & 0xff)) {
        if (x12 == 0) {
          char_dimensions[count][x12] = cx;
          /* Adjust to compensate for the x2 scaled-up font with double width
           * markers */
...
SDL_Surface *TTF_RenderUTF8_Shaded(SDL_Surface *font, const char *text,
                                   SDL_Color fg, SDL_Color bg) {
  int code_point_size, text_offset,
      character_spacing = 1; //-video.field_size / 16;
  ...
  while (*(utf8 + text_offset) != 0) {
    ...
    /* Point to next code point being careful not to go past the terminating
     * zero */
    ...
    while (code_point_size--) {
      ...
      if (*(utf8 + text_offset) == 0) {
        rendered_text_width -= character_spacing; /* We don't want character
                                                     spacing adjustments after
                                                     the last char */
        break;
      }
    }
  ...
  rendered_text = SDL_CreateRGBSurface(0, rendered_text_width,
                                       rendered_text_height, 32, 0, 0, 0, 0);
  if (rendered_text != NULL) {

    /* Fill it with the background colour */
    SDL_FillRect(rendered_text, NULL,
                 SDL_MapRGB(rendered_text->format, bg.r, bg.g, bg.b));
    ...
    while (*(utf8 + text_offset) != 0) {
      ...
      if (*(utf8 + text_offset) >= 0xc2 && *(utf8 + text_offset) <= 0xdf) {
        code_point_size = 2;
      } else if (*(utf8 + text_offset) >= 0xe0 &&
                 *(utf8 + text_offset) <= 0xef) {
        code_point_size = 3;
      } else if (*(utf8 + text_offset) >= 0xf0 &&
                 *(utf8 + text_offset) <= 0xf4) {
        code_point_size = 4;
      } else {
        code_point_size = 1;
      }
    ...
    if (SDL_MUSTLOCK(rendered_text))
      SDL_LockSurface(rendered_text);
    for (filly = 0; filly < rendered_text->h; filly++) {
      for (fillx = 0; fillx < rendered_text->w; fillx++) {
        if (get_pixel(rendered_text, fillx, filly) ==
            SDL_MapRGB(rendered_text->format, FONT_CHAR_COLOUR >> 16 & 0xff,
                       FONT_CHAR_COLOUR >> 8 & 0xff, FONT_CHAR_COLOUR & 0xff)) {
    ...
    if (SDL_MUSTLOCK(rendered_text))
      SDL_UnlockSurface(rendered_text);
...
SDL_Rect get_code_point_srcrect(SDL_Surface *font, const unsigned char *utf8) {
  ...
  srcrect.y = 1;       // video.field_size / 16;
 ...
 * If surfaces must be locked/unlocked then the caller is responsible for this.
 *
 * The pixel being put should be mapped to the surface format first using
 * SDL_MapRGB() */
